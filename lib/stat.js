...
// data - object with stat information/ Should have properties: currentWord, timeEnd, timeStart, guesses
...
// wordObject[0].guessed - how many times some word has been guessed (by all users)
  setStat: function setStat (data) {
    collection.find({'word': data.currentWord}, function (err, wordObject) {
      if (err) return console.log(err);
      var time = Math.floor((data.timeEnd - data.timeStart)/1000);
      var newAvgTime = average(wordObject[0].time_avg, wordObject[0].guessed, time);
      var newAvgGuesses = average(wordObject[0].amountOfGuesses, wordObject[0].guessed, data.guesses);
